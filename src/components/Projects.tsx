...
import { projectKey, getProjectDetails } from '../i18n/utils'
...
const ProjectModal = ({ project, isOpen, onClose }: ProjectModalProps) => {
  ...
  const projectDescription = t(projectKey(project.id, 'description'))
  const projectDetails = getProjectDetails(t, project.id)
    ...
    >
      <div className="bg-[#0f1422] border border-[#1e293b] rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="p-6">
          ...
          <p className="text-neutral-300 mb-6">{projectDescription}</p>

          <div className="mb-6">
            <h4 className="text-sm font-semibold text-primary-400 mb-2">{t('modalDetails')}</h4>
          ...
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-primary-400 mb-2">{t('modalStack')}</h4>
  ...
  return (
    <>
      <article
        ...
        tabIndex={0}
        role="button"
        aria-label={`${t('projectViewDemo')}: ${projectTitle}`}
      >
        {/* Badges */}
        ...
        <div className="aspect-video relative overflow-hidden bg-[#0a0e27]">
          {projectImage ? (
            <img
              src={projectImage}
              alt={`${t('projectsTitle')}: ${projectTitle}`}
        ...
        <div className="p-6">
          <p className="text-neutral-300 mb-4 line-clamp-2">{projectDescription}</p>
          ...
          <div className="flex gap-3">
            ...
            {project.github && (
              <a
                ...
                onClick={(e) => e.stopPropagation()}
                className="inline-flex items-center justify-center px-4 py-2 border border-[#1e293b] text-neutral-300 text-sm font-semibold rounded-lg hover:border-primary-500/50 hover:bg-[#1e293b] transition-all hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-[#0f1422]"
                aria-label={`${t('contactGitHub')}: ${projectTitle}`}
              >
            ...
            <button
              ...
              className="inline-flex items-center justify-center px-4 py-2 border border-[#1e293b] text-neutral-300 text-sm font-semibold rounded-lg hover:border-primary-500/50 hover:bg-[#1e293b] transition-all hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-[#0f1422]"
              aria-label={`${t('projectDetails')}: ${projectTitle}`}
            >
              {t('projectDetails')}
            </button>
          </div>
